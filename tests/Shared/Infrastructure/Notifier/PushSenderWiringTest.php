 ...
 *
 * Il ne mocke rien : `self::getContainer()->get(PushSender::class)` est le vrai
 * `ExpoPushSender`, avec le vrai `TexterInterface` compilé du conteneur. S'il envoyait
 * réellement sur le réseau, ce test le ferait — c'est la garantie qu'aucune suite
 * PHPUnit ne peut appeler Expo, prouvée plutôt qu'affirmée.
 */
final class PushSenderWiringTest extends ApiTestCase
{
    public function testResolvesFromTheContainerAndSendsWithoutReachingTheNetwork(): void
    {
        ...
        $tickets = $sender->send($bob->id, new PushNotification('Titre', 'Corps', NotificationCategory::GuildActivity, 'wiring'));
    ...
    public function testAPlayerWithNoDeviceGetsNoTicket(): void
    {
        ...
        self::assertSame([], $sender->send($bob->id, new PushNotification('Titre', 'Corps', NotificationCategory::GuildActivity, 'wiring')));
    }
}
