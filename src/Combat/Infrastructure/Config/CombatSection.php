 ...
 * des ennemis et des boss (#219).
 *
 * Il vit dans `Combat` et non dans `Shared` : c'est ce module qui lit la section. Le schéma
 * dit la **forme** — trois blocs, `fighter`, `enemies` et `bosses` — et délègue la cohérence
 * à deux objets du domaine : {@see CombatRules} pour les coefficients, {@see EnemyCatalog}
 * pour les deux catalogues d'adversaires. Même geste que `CommunitySection`, qui délègue à
        ...
        $tree->getRootNode()
            ->children()
                ...
                ->arrayNode('enemies')
                    ...
                    ->end()
                ->end()
                // Un boss ne se tire jamais automatiquement (#219) : pas de
                // `requiresAtLeastOneElement()`, et `minimum_level` au lieu d'un palier —
                // voir le docblock d'`EnemyCatalog` pour pourquoi les deux listes restent
                // séparées plutôt qu'un drapeau sur `enemies`.
                ->arrayNode('bosses')
                    ->defaultValue([])
                    ->arrayPrototype()
                        ->children()
            ...
            ->validate()
                ->always(static function (array $values): array {
                     ...
                     *     bosses: list<array{key: string, minimum_level: int, hp: int, damage: int, mitigation_permille: int, extra_turn_permille: int, dodge_permille: int}>,
                    ...
                    try {
                        ...
                        new EnemyCatalog($values['enemies'], $values['bosses']);
